...
def get_df(config):
    rating_df = pd.read_csv(config['ratingdf_path'], sep='::', header=None)
...
def enrich_data_by_vl(data, node_num, image_num, text_num, iid_dict, asin_image_dict, asin_text_dict):
    ...
    for seq in seqs:
        ...
        text_seq = []
        for i in seq:
            itemid = iid_dict[i] # in asin_image_dict, the key-itemid is str
            if str(itemid) in asin_image_dict:
                if type(asin_image_dict[str(itemid)]) == list:
                    image_c = [ c+node_num for c in asin_image_dict[str(itemid)] ]
                else:
                    image_c = [asin_image_dict[str(itemid)]+node_num] # asin_image_dict[itemid] is an int
            else:
                image_c = []
...
if __name__=='__main__':
    ...
    config['ratingdf_path'] = '/ssd1/holdenhu/ML_dataset/ml-1m/raw/ratings.dat'
